...
[![](https://i.ytimg.com/vi/8Mj2u-iZkvA/maxresdefault.jpg)](http://www.yes24.com/Product/Goods/66820470)
...
.pull-left[
```{r, out.width="100%"}
include_graphics("http://image.yes24.com/Goods/61958728/800x0")
```
]
...
```{r, out.width="85%"}
include_graphics("http://mblogthumb3.phinf.naver.net/20160322_234/chosun_ccg_1458628199496F2Npc_JPEG/111222.jpg?type=w800")
```
...
```{r, out.width="70%"}
include_graphics("http://postfiles16.naver.net/20100109_191/corazon27_1263017643108wXiID_jpg/%EA%B7%B8%EB%A6%BC_%EA%B7%B8%EB%A6%AC%EB%8A%94_%EC%86%90_corazon27.jpg?type=w3")
```
...
```{r, out.width="50%"}
include_graphics("http://image.yes24.com/goods/4357464/L")
```
...
```{r, out.width="50%"}
include_graphics("http://cfs10.blog.daum.net/image/7/blog/2007/09/22/21/58/46f5118794ac3&filename=ASCII_merlin_detail_00031-ybh333y.gif")
```
...
```{r, out.width="50%"}
include_graphics("https://dcimg4.dcinside.co.kr/viewimage.php?id=2cb2d521e4df3d&no=24b0d769e1d32ca73fee82fa11d02831f7b1fdb67b76037d9d6c024c247c057fb2f51774dc6693417620532f563a069f071df1a1d03e9bb6f101c258d73d5a26c77505d4942033402b62fdd90cab4faaa0c55375b6ddd5b9d882")
```
...
* 만약 $G$ 가 **거짓**이라면: $G$ 는 증명가능 즉, **참**인 명제이다. 모순.

* 따라서 $G$ 는 참인 명제 & 그 내용은 ** $G$는 증명 불가능하다.**
...
* 수학명제 $G$ 가 거짓이라면
    ...
    + 대응하는 메타수학의 명제도 거짓 $\rightarrow$ **괴델수 $g$ 를 갖는 명제** 는 증명 가능하다(참이다).
...
* 따라서 수학명제 $G$ 는 참이다. 
    ...
    + 대응하는 메타수학의 명제도 참 $\rightarrow$ **괴델수 $g$ 를 갖는 명제** 는 증명 불가능하다.
...
--
$$G: ``\textbf{괴델수 sub} (n, 17, n) \textbf{ 을 갖는 명제}" \text{ 는 증명 불가능하다.}$$
...
에 등장하는 **변수 $y$ 들에 전부 숫자 $n$ 를 대입**해서 만들어진 명제!!


---
# G와 g를 찾았으므로 증명끝!

$$G: ``\textbf{괴델수 } g \textbf{ 를 갖는 명제}" \text{ 는 증명 불가능하다.}$$

에 **대응하는 수학명제의 괴델수가 $g$** 라면?

* 메타수학명제 $G$ 가 거짓이라면 모순이 발생한다.

    + 대응하는 수학명제인 **괴델수 $g$ 를 갖는 명제** 도 거짓   
    
    + 한편 $G$의 부정은 참 $\rightarrow$ **괴델수 $g$ 를 갖는 명제** 는 증명 가능하다(참이다).


* 따라서 메타수학명제 $G$ 는 참이다. 

    + 대응하는 수학명제인 **괴델수 $g$ 를 갖는 명제** 도 참
    
    + 한편 $G$는 참 $\rightarrow$ **괴델수 $g$ 를 갖는 명제** 는 증명 불가능하다.


**괴델수 $g$를 갖는 수학명제**는 참이지만 증명 불가능하다!!
...
.large[
* 제 2 불완전성까지 증명되며, **수학 산술체계의 무모순성**(힐베르트 23문제 중 2번 문제)에 대한 힐베르트의 꿈은 박살난다. 
